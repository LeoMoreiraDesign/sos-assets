      ...
      <header className="p-6 md:p-8 flex justify-between items-center bg-white/60 backdrop-blur-lg border-b border-white/50 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto w-full flex items-center justify-between">
            ...
            {currentPage === 'products' ? (
              <>
                <button 
                  ...
                  className="flex items-center gap-1.5 text-xs font-semibold text-neutral-700 bg-white/80 border border-neutral-200/50 px-3 py-1.5 rounded-lg shadow-sm active:scale-95 transition-transform"
                  aria-label={isMobileFilterOpen ? 'Fechar Filtros' : 'Abrir Filtros'}
                >
                  <ListFilter size={14} /> 
                  {isMobileFilterOpen ? 'Fechar' : 'Filtros'}
                </button>
                <button 
                  onClick={() => navigateTo('support')}
                  className="flex items-center gap-1.5 text-xs font-semibold text-[#0071e3] bg-[#0071e3]/5 border border-[#0071e3]/10 px-3 py-1.5 rounded-lg active:scale-95 transition-transform"
                >
                  Suporte
                </button>
              </>
              ...
              <button 
                ...
                className="flex items-center gap-1.5 text-xs font-semibold text-[#0071e3] bg-[#0071e3]/5 border border-[#0071e3]/10 px-4 py-1.5 rounded-full active:scale-95 transition-transform"
              >
                Ver Produtos
              </button>
            )}
          ...
          <nav className="hidden md:flex items-center gap-8 text-sm font-semibold text-neutral-500">
            <button 
              onClick={() => navigateTo('products')} 
              className={`transition-colors ${currentPage === 'products' ? 'text-apple-black' : 'hover:text-apple-black'}`}
            >
              Produtos
            </button>
            <button onClick={() => navigateTo('products')} className="hover:text-apple-black transition-colors">Seminovos</button>
            <button 
              onClick={() => navigateTo('support')} 
              className={`transition-colors ${currentPage === 'support' ? 'text-apple-black' : 'hover:text-apple-black'}`}
            >
              Assistência
            </button>
            <a href="https://api.whatsapp.com/send?phone=5511981765177" target="_blank" rel="noopener noreferrer" className="hover:text-apple-black transition-colors" aria-label="Falar pelo WhatsApp">Contato</a>
          </nav>
        </div>
      </header>
